...
| `advisor.agents`      | array   | `[]`    | Agent definition names that observe the main session as advisors; empty falls back to the default advisor on `modelRoles.advisor`, and the reserved entry `default` selects that built-in advisor explicitly. Subagents get advisors from their definition's `advisors:` frontmatter instead. See [Advisor, agent definitions, and WATCHDOG.md](./advisor-watchdog.md). |
