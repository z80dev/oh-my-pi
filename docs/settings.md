...
| `advisor.agents`      | record  | `{}`    | Advisor roster observing the main session: `name → model override` entries (`null` = no override). Empty falls back to the default advisor on `modelRoles.advisor`, and the reserved entry `default` selects that built-in advisor explicitly, with a per-entry model override when set. Subagents get advisors from their definition's `advisors:` frontmatter instead, using the same roster shape. See [Advisor, agent definitions, and WATCHDOG.md](./advisor-watchdog.md). |
